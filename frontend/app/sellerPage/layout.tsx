...
const SellerPageLayout = ({ children }: { children: React.ReactNode }) => {
  ...
  // Redirect to /sellerPage/shop if the current path is /sellerPage
  useEffect(() => {
  ...
  // Automatically set the active menu based on the pathname
  useEffect(() => {
...
//
// const SellerPageLayout = ({ children }: { children: React.ReactNode }) => {
//   const [activeMenu, setActiveMenu] = useState<string>("shop");
...
//   };
//
//   // Set active menu on pathname changeMore
//   useEffect(() => {
//     if (pathname.includes("/shop")) {
//       setActiveMenu("shop");
//     } else if (pathname.includes("/about")) {
...
//               className={`flex items-center s// const fetchCategories = async () => {
//   //   try {
//   //     const res = await fetch(categoryUrl);
//   //     const data = await res.json();
//   //     if (data?.data?.length) {
...
//   //     setCategories([]);
//   //   }
//   // };pace-x-2 bg-gray-300 text-gray-700 font-medium px-4 py-2 rounded-lg hover:bg-gray-400 transition`}
//             >
//               <svg
