...
import { CategoryData, getColumns } from './columns';

export default function DataTable({ data }: { data: CategoryData }) {
  const { data: session } = useSession();
  const token = session?.user?.token;

  const columns = getColumns(data.category.name); // Main category as header
  const table = useReactTable({
    data: data.subCategories, // Directly passing subCategories
    columns,
